<template>
    <div>
        ...
        <v-layout row wrap class="mt-1" v-else-if="userType == 1">
            <v-flex xs12 sm4 md3 v-for="item of data" :key="item[0]" >
                <v-card elevation="7" :to="{name: 'CursoImpartidoJefe', params:{CursoImpartidoJefe: item.courseName}}" >
                    <v-toolbar dark color="green lighten-1" card v-if="item.state=='Por empezar'">
                        <span class="subheading">{{item.courseName}}</span>
                    </v-toolbar>
                    <v-toolbar dark color="yellow darken-2" card v-else-if="item.state=='Cursando'">
        ...
        <v-layout row wrap class="mt-1" v-else-if="userType == 4">
            <v-flex xs12 sm4 md3 v-for="item of data" :key="item[0]" >
                <v-card elevation="7" :to="{name: 'CursoImpartidoExt', params:{CursoImpartidoExterno: item.courseName}}" >
                    <v-toolbar dark color="green lighten-1" card v-if="item.state=='Por empezar'">
                        <span class="subheading">{{item.courseName}}</span>
                    </v-toolbar>
                    <v-toolbar dark color="yellow darken-2" card v-else-if="item.state=='Cursando'">
